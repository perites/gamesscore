...
def user_list_get(user_id):
    ...
    info = user_id.get_fields({"show_info": 1, "games": 1, "all_criterias": {"order": 1}})

    info["all_criterias"] = info["all_criterias"]["order"]
    ...
    # print(games_name)
    games_additional_info = list(user_id.get_games(games_name, {"display_name": 1, "image": 1}))
    # print(games_additional_info)
    info["games"] = user_id.get_valid_games(info["games"], info["all_criterias"], games_additional_info, user_status, user_tag)
...
def user_settings_get():
    ...
    criterias = info["all_criterias"]

    user_show_info = info["show_info"]
    rest_criterias = [el for el in basic_criterias if el not in user_show_info]

    return render_template("user_2_settings.html", criterias=criterias, steam_form=steam_form, rest_criterias=rest_criterias, user_show_info=user_show_info)
...
def user_settings_post():
    ...
    match form['action']:
        case "change_criteria":
            ...
            current_user.change_criteria(form["change_criteria_name"], form["new_criteria_name"], new_values)
...
def game_page_get(game, game_id):
    if current_user.is_authenticated:
        user_info = uh.users_collection.find_one({"_id": current_user.id}, {"games": {"$elemMatch": {"game_id": game_id}}, "favorites.games": 1})
        print(user_info)
...
def game_page_post(game_id):
    if request.form.get("add_to_favorites"):
...
def change_user_game_page_get(game_additional_info, game_id):
    ...
    if not info.get("games"):
        ...
        info = uh.users_collection.find_one({"_id": current_user.id}, {"games": {"$elemMatch": {"game_id": game_id}}, "all_criterias": 1})
    ...
    print(info["game"]["criterias"], "-----------------------------")
    info["game"] = current_user.get_valid_games([info["game"]], info["all_criterias"], [game_additional_info])[game_id]
    ...
    form.tags.default = ";".join(info["game"]["info"].get("Tags")) + ";" if info["game"]["info"].get("Tags") else None
    form.process()
...
def change_user_game_page_post(game_id):
    ...
    all_criterias = request.args.to_dict()["all_criterias"]
    valid_game_criterias = {}
    for criteria in all_criterias[12:-3].split("', '"):
